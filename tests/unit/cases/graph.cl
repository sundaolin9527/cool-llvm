...
class Main inherits Parse {
   main() : Object {
      (let g : Graph <- new Graph in
         {
            g.add_vertice(
               (new Vertice).init(1).add_out(new Edge.init(1, 2, 100))
            );
            g.add_vertice(
               (new Vertice).init(2)
                  .add_out(new Edge.init(2, 3, 200))
                  .add_out(new Edge.init(2, 1, 150))
            );
            g.add_vertice(
               (new Vertice).init(3).add_out(new Edge.init(3, 2, 10))
            );
            g.add_vertice(
               (new Vertice).init(4)
                  .add_out(new Edge.init(4, 3, 55))
                  .add_out(new Edge.init(4, 5, 100))
            );
            g.add_vertice(
               (new Vertice).init(5)
                  .add_out(new Edge.init(5, 1, 1))
                  .add_out(new Edge.init(5, 2, 2))
                  .add_out(new Edge.init(5, 3, 3))
                  .add_out(new Edge.init(5, 4, 4))
                  .add_out(new Edge.init(5, 5, 5))
            );
	    g.print_V();
            g.print_E();
         }
